...
def evaluate_sft_qa(cfg: Any) -> Path:
    ...
    test_safe_file = Path(cfg_dict.get("test_safe_file") or sft_dir / "sft_test_safe.jsonl")
    ...
    test_safe_rows = _sample_rows(list(iter_jsonl(test_safe_file)), max_examples, seed)
    attack_rows = _sample_rows(list(iter_jsonl(attack_file)), max_examples, seed + 1)
    test_safe_results = _evaluate_rows(
        model=model,
        tokenizer=tokenizer,
        rows=test_safe_rows,
    ...
    metrics = {
        "model": str(model_dir),
        "sft_dir": str(sft_dir),
        "test_safe_file": str(test_safe_file),
        "attack_file": str(attack_file),
        "chat_template_path": str(chat_template_path),
        "batch_size": batch_size,
        "max_new_tokens": max_new_tokens,
        "test_safe": _summarize_results(test_safe_results),
        "attack": _summarize_attack(attack_results),
    }
    _write_jsonl(output_dir / "sft_test_safe_predictions.jsonl", test_safe_results)
